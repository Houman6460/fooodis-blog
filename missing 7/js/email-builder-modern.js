...
document.addEventListener('DOMContentLoaded', function() {
  console.log('💡 Initializing Modern Email Builder...');
  ...
  // Add the exact same block toolbar that already exists in the application
  function addDragHandles(block) {
    // Remove any existing handles or buttons first to avoid duplicates
    const existingButtons = block.querySelectorAll('.block-handle, .block-action-btn, .block-actions, .block-toolbar');
    existingButtons.forEach(btn => btn.remove());
    
    // Create the toolbar with exactly the same structure as in email-block-structure.js
    const toolbar = document.createElement('div');
    toolbar.className = 'block-toolbar';
    
    // Use the exact same HTML structure and Material Icons as the existing implementation
    toolbar.innerHTML = `
    ...
    // Add toolbar to the beginning of the block (same as original implementation)
    block.insertBefore(toolbar, block.firstChild);
    
    // Add the same event listeners as the original implementation
    toolbar.querySelector('.block-settings').addEventListener('click', (e) => {
      e.stopPropagation();
      selectBlock(block);
    ...
    toolbar.querySelector('.block-duplicate').addEventListener('click', (e) => {
      e.stopPropagation();
      const newBlock = block.cloneNode(true);
      // Ensure IDs are unique
      if (newBlock.id) {
        newBlock.id = newBlock.id + '-copy-' + Date.now();
      }
      // Insert after the current block
      if (block.parentNode) {
        block.parentNode.insertBefore(newBlock, block.nextSibling);
        // Re-initialize the new block
        addDragHandles(newBlock);
        makeBlockDraggable(newBlock);
      }
    });
    
    toolbar.querySelector('.block-delete').addEventListener('click', (e) => {
      e.stopPropagation();
    ...
    // Make block position relative for proper toolbar positioning if not already
    if (block.style.position !== 'relative') {
      block.style.position = 'relative';
    }
    
    // Add click handler directly to the block for editing
    block.addEventListener('click', function(e) {
      // Don't trigger if clicking on toolbar
      if (!e.target.closest('.block-toolbar') && (e.target === block || block.contains(e.target))) {
