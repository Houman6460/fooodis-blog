    ...
    window.FoodisChatbot = {
        ...
        setupDropdownHandlers: function() {
            ...
            const setupDropdown = (lang) => {
                ...
                systemUsageSelect.addEventListener('click', (e) => {
                    console.log(`🖱️ Dropdown clicked for ${lang}`);
                    e.stopPropagation();
                });
                systemUsageSelect.addEventListener('focus', (e) => {
                    console.log(`🎯 Dropdown focused for ${lang}`);
                    e.target.style.borderColor = '#007bff';
                });
                systemUsageSelect.addEventListener('blur', (e) => {
                    e.target.style.borderColor = '#ddd';
                });
                
                // Remove any conflicting overlays that might block the dropdown
                const parentForm = systemUsageSelect.closest('form') || systemUsageSelect.closest('.chatbot-registration-content');
                if (parentForm) {
                    const overlays = parentForm.querySelectorAll('[style*="position: absolute"], [style*="position: fixed"]');
                    overlays.forEach(overlay => {
                        if (overlay !== systemUsageSelect && overlay.style.zIndex && parseInt(overlay.style.zIndex) > 10000) {
                            overlay.style.zIndex = '9999';
                            console.log(`📉 Reduced overlay z-index to prevent dropdown blocking`);
                        }
                    });
                }
        ...
        async submitRegistration() {
            ...
            // Store all user data for conversation records
            this.userEmail = email;
            this.userName = name;
            this.userPhone = phone;
            this.userType = systemUsage;
            this.restaurantName = restaurantName;
            this.formLanguage = lang;
            
            // Determine flag based on language selection
            const languageFlag = this.getLanguageFlag(lang);
            this.languageFlag = languageFlag;
            this.displayFlag = languageFlag;
            ...
            try {
                ...
                // Immediately update conversation data with user info and store
                this.updateConversationUserData();
                this.storeConversation();
                
                // Also force refresh of dashboard if it exists
                if (window.chatbotManager) {
                    setTimeout(() => {
                        window.chatbotManager.loadConversationsFromServer();
                    }, 500);
                }
        ...
        // Get language flag based on language code
        getLanguageFlag: function(languageCode) {
            ...
            return flagMap[languageCode?.toLowerCase()] || null;
        },
        ...
        autoSaveConversation: function() {
            ...
            const conversationData = {
                ...
                languageFlag: this.languageFlag || null,
                displayFlag: this.displayFlag || null,
                language: this.currentLanguage || 'en',
                formLanguage: this.formLanguage || null,
        ...
        // Update conversation user data immediately after registration
        updateConversationUserData: function() {
            console.log('🔄 Updating conversation user data after registration');
            
            // Update current conversation in localStorage
            const conversations = JSON.parse(localStorage.getItem('fooodis-chatbot-conversations') || '[]');
            const currentConvIndex = conversations.findIndex(conv => conv.conversationId === this.conversationId);
            ...
            if (currentConvIndex >= 0) {
                ...
                localStorage.setItem('fooodis-chatbot-conversations', JSON.stringify(conversations));
                console.log('✅ Updated existing conversation with user data:', conversations[currentConvIndex]);
            } else {
                console.log('⚠️ Current conversation not found in localStorage for update');
            }
        },
        ...
        getDepartmentDisplayName: function(department) {
            const departmentNames = {
                ...
                'sales': this.getCurrentLanguage() === 'sv' ? 'Försäljning' : 'Sales',
