...
(function() {
    ...
    function initDirectOverride() {
        console.log("Direct Media Selector Override: Initializing");
        ...
        document.querySelectorAll('[data-action="media"], .select-image-btn, .media-button, [onclick*="openMediaLibrary"], [onclick*="selectMedia"]').forEach(btn => {
            ...
            if (originalOnclick) {
                ...
                try {
                    ...
                    console.error("Error extracting target ID:", err);
                }
    ...
    function updateMediaUI(mediaItems) {
        ...
        mediaItems.forEach(item => {
            ...
            let thumbnailSrc = item.thumbnail || item.path || '#';
            const fileName = item.name || (item.path ? item.path.split('/').pop() : 'Media item');
            
            // If path starts with images/ or media/, try to use it directly
            if (thumbnailSrc === '#' || !thumbnailSrc.startsWith('http')) {
                // Create a colored background as fallback
                const placeholderColor = getColorForFolder(item.folder || 'uncategorized');
                thumbnailSrc = createColorPlaceholder(placeholderColor);
            }
            
            mediaItem.innerHTML = `
                <div class="media-thumbnail">
                    <img src="${thumbnailSrc}" alt="${fileName}" data-placeholder="${thumbnailSrc.startsWith('data:') ? 'true' : 'false'}">
                </div>
    ...
    /**
     * Get color for a folder type
     */
    function getColorForFolder(folder) {
        const colors = {
            food: '#e74c3c',      // Red
            restaurant: '#2ecc71', // Green
            people: '#f39c12',     // Orange
            uncategorized: '#3498db' // Blue
        };
        
        return colors[folder] || colors.uncategorized;
    }
        ...
        return [
            {
                ...
                folder: 'food',
                path: 'images/media/cappuccino-or-latte-coffee-with-heart-art.jpg'
            },
            {
                id: 'img2',
                name: 'japanese-tea-2024-04-08-18-08-34-utc.jpg',
                folder: 'food',
                path: 'images/media/japanese-tea-2024-04-08-18-08-34-utc.jpg'
            },
            {
                id: 'img3',
                name: 'white-cup-of-tasty-cappuccino.jpg',
                folder: 'food',
                path: 'images/media/white-cup-of-tasty-cappuccino.jpg'
            },
            {
                id: 'img4',
                name: 'hot-coffee-latte-art-on-wooden-table.jpg',
                folder: 'food',
                path: 'images/media/hot-coffee-latte-art-on-wooden-table.jpg'
            },
            {
                id: 'img5',
                name: 'appetizing-soup-served-with-herbs.jpg',
                folder: 'food',
                path: 'images/media/appetizing-soup-served-with-herbs.jpg'
            },
            {
                id: 'img6',
                name: 'restaurant-interior.jpg',
                folder: 'restaurant',
                path: 'images/media/restaurant-interior.jpg'
            },
            {
                id: 'img7',
                name: 'chef-cooking.jpg',
                folder: 'people',
                path: 'images/media/chef-cooking.jpg'
            },
            {
                id: 'img8',
                name: 'chef-decorating.jpg',
                folder: 'people',
                path: 'images/media/chef-decorating.jpg'
            },
            {
                id: 'img9',
                name: 'a-full-bag-of-brown-coffee-beans.jpg',
                folder: 'food',
                path: 'images/media/a-full-bag-of-brown-coffee-beans.jpg'
            }
        ];
    }
    ...
    function selectMedia(item) {
        console.log("Direct Selector Override: Selected media", item);
        ...
        if (currentTargetInputId) {
            ...
            if (input) {
                // Set the value to the path or name
    ...
    function updatePreview(inputId, item) {
        ...
        // If path doesn't start with http, it might be a relative path
        if (thumbnailSrc === '#' || !thumbnailSrc.startsWith('http')) {
            // Create a colored background as fallback
            const placeholderColor = getColorForFolder(item.folder || 'uncategorized');
            thumbnailSrc = createColorPlaceholder(placeholderColor);
        ...
        if (preview) {
            if (preview.tagName === 'IMG') {
                ...
                preview.setAttribute('data-placeholder', thumbnailSrc.startsWith('data:') ? 'true' : 'false');
            } else {
                ...
                img.alt = item.name || '';
                img.setAttribute('data-placeholder', thumbnailSrc.startsWith('data:') ? 'true' : 'false');
            }
        }
    }
